...
For example, synthetic fields have been used extensively to validate methods and measure turbulence in the interstellar medium [@Brunt.Heyer02; @Miville-Deschenes.etal03; @Esquivel.etal03; @Ossenkopf.etal06] and similarly for the ICM [@Vogt.Ensslin05; @ZuHone.etal16; @XrismCollaboration.etal25].

`kea` provides methods to produce (monofractal and multifractal) synthetic fields with correlations parameterized by power spectral density. These fields are synthesized in Fourier-space and Fourier-transformed to provide real-space fluctuation fields [@Barnsley.etal88; @Lakhal.etal25]. For examples, shown in \autoref{fig:example_synthesis} are one, two, and three-dimensional monofractal fields. Thus, the below PSD estimators can be tested for validity with known/expected forms, and their performance tested under constraints of noise and missing data.

## Fourier Convention

The data we obtain from real observations is not continuous data, but rather, a discrete function that is obtained by sampling an underlying continuous function, $y_\mathrm{c}$, at an interval $\Delta x$. This gives the discrete function $y[n]$ and its Fourier transform $\widehat{y}[m]$, defined as the following:
$$
...
$$
where the summations are over all $\mathbf{m},\mathbf{n}$ e.g., $\sum_{\mathbf{n}} = \sum_{n_1=0}^{N_1-1} \sum_{n_2=0}^{N_2 - 1} \dots \sum_{n_D=0}^{N_D - 1}$.
...
We discuss the class of functions termed the "lag-function" which use the same backend i.e., performing some operation (where the ACF performs multiplication, and the SF performs subtraction) on data that has been shifted by a lag distance.
...
![Capabilities of `Kea` for structure function scaling analysis. The package is used to generate 1D synthetic monofractal and multifractal fields, calculate structure functions from order $p=1$ to $p=8$, and extract the corresponding power-law slopes.\label{fig:SF_scaling}](plots/SF_scaling.png)
...
`Kea` also facilitates the calculation of higher-order scale-dependent statistics using the order parameter $p$. This can be used to e.g., evaluate mono- and multi-fractal scaling behaviour as shown in \autoref{fig:SF_scaling}; an approach fundamental to modern turbulence studies [@Frisch95; @Biskamp03].
...
`Kea` provides discrete implementations of several spectral estimation techniques. An application of these methods to a synthetic timeseries is demonstrated in \autoref{fig:PSD_example}. We introduce each of the supported methods below.
...
$$
Note that with the unbiased normalization ($\widetilde{N}[m] = N-m-1$), $\widehat{Q}[\mathbf{m}]$ is not guaranteed to be positive semi-definite, possible resulting in negative spectral estimates which is undesirable in most applications [@Stoica.Moses05].

### Equivalent Spectrum

To estimate a power spectrum using the structure function, first, one must:
...
Then, the following algorithm is performed [@Bishop.etal26]:
...
- Estimate an "uncorrected" equivalent spectrum, $\widetilde{\mathcal{E}}^{\mathrm{ESF}}[m]$ using the following relationship:
    ...
    $$
    where $\frac{\Delta \overline{S}^{(2)}[m]}{\Delta \left(n \Delta x \right)}$ represents a finite-difference estimate of the lag-space derivative of the structure function. In the above equation, $m$ still represents the lag-shift. To convert $m$ to a wavenumber, a factor $b$ is required.
...
`kea` performs the above algorithm and returns both the "uncorrected" and "debiased" spectral estimates along with the associated (equivalent) wavenumbers. For the mathematical justification and validation of this method of a range of datatypes see [@Bishop.etal26].
...
$$
For $k = \left\{\frac{2\pi}{L},\, \dots,\, \frac{\pi N}{L} \right\}$, then $\sigma = \left\{\frac{b L}{\pi N},\, \dots,\, \frac{bL}{2\pi}\right\}$ and $o = \left\{\frac{b}{\pi},\, \dots, \,\frac{bN}{2\pi} \right\}$. Similar to the ESF method, $b=\pi$ would be a natural conclusion to arrive at based on a signal in a domain $L$, however, it has been suggested otherwise [@Bishop.etal26]. A common interpretation is $b=\sqrt{2}$ [@Arevalo.etal12].
...
Following the continuous definitions the ACF and SF are typically represented via averaging circular/spherical shells\footnote{In the case of 1D data, there are exactly 2 values for each $\left| \mathbf{k} \right|$: the positive and negative value.}. Whereas, the angle-integrated ($\mathcal{E}[b]$) and angle-averaged ($\overline{E}[b]$) spectra are found when $\widetilde{\sum}$ in \autoref{eqn:binning} is the summation and average, respectively:
$$
