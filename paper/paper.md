...
We introduce the class of functions termed the "lag-function" which use the same backend i.e., performing some operation (where the ACF performs multiplication, and the SF performs subtraction) on data that has been shifted by a lag distance.
...
`kea` performs the above algorithm and returns both the "uncorrected" and "debiased" spectral estimates along with the associated (equivalent) wavenumbers. This method has been validated on a range of data-types [@Bishop.etal26].
