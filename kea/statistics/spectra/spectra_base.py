...
def transform_spectrum(
    ...
    Args:
        ...
        fek (np.ndarray): Spectrum of `current_type`
        orig_dim (int): Dimension of the original data $D$
...
def convert_normalization_convention(
    ...
    """convert_normalization_convention(kvec, fek, orig_dim, current_norm, new_norm, spectrum_type)\n
    ...
    if spectrum_type == SpectrumType.MODAL or spectrum_type == SpectrumType.AVERAGED:
        CONVERSION_TABLE = {
            (FourierNorm.T, FourierNorm.I): twopi**(orig_dim),
            (FourierNorm.T, FourierNorm.C): twopi**(orig_dim),
        ...
        CONVERSION_TABLE = {
            ...
            (FourierNorm.T, FourierNorm.C): twopi**(orig_dim),
            (FourierNorm.I, FourierNorm.C): twopi**(1-orig_dim),
        }
    elif spectrum_type == SpectrumType.AMPLITUDE:
        CONVERSION_TABLE = {
            (FourierNorm.T, FourierNorm.I): 1.,
            (FourierNorm.T, FourierNorm.C): twopi**(orig_dim),
            (FourierNorm.I, FourierNorm.C): twopi**(orig_dim/2.),
        }
    else:
